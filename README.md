...
　「MSX快速マシン語ゲーム集」に掲載されている「ジグソーセット」は、機械語のサブプログラムから選んだ絵のデータをBIOSのCMTアクセスルーチンをコールしてロードする仕様になっています。
...
#### 注意
　アプリケーションからSDアクセスルーチンをコールするためにSDアクセスルーチンをFDのワークエリアにコピーしています。FDとは共存できないので機械語多段ロードでBIOSをコールしているプログラムをテストするときは必ずFDを切り離して起動してください。
